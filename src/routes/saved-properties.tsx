...
import { fetchPublishedProperties, fetchSavedProperties } from "@/lib/property-service";
...
function SavedProperties() {
  ...
  // Authenticated users: fetch via favourites table. Guests: fetch all published then filter by localStorage ids.
  const authedQuery = useQuery({
    queryKey: ["saved", user?.id],
    queryFn: () => (user ? fetchSavedProperties(user.id) : Promise.resolve([])),
    enabled: !!user,
  });
  const guestQuery = useQuery({
    queryKey: ["saved-guest"],
    queryFn: () => fetchPublishedProperties(),
    enabled: !user,
  });

  const saved = useMemo(() => {
    if (user) return authedQuery.data ?? [];
    return (guestQuery.data ?? []).filter((p) => ids.includes(p.id));
  }, [user, authedQuery.data, guestQuery.data, ids]);

  useEffect(() => setPage(1), [saved.length]);

  const isLoading = user ? authedQuery.isLoading : guestQuery.isLoading;
    ...
    <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
      ...
      {isLoading ? (
        ...
        <>
          ...
          <Pagination page={page} totalPages={totalPages} onChange={setPage} totalItems={saved.length} perPage={PER_PAGE} />
        </>
      )}
    </div>
  );
